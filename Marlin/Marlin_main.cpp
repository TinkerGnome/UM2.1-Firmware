...
void process_command(const char *strCmd)
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      feedrate = 0.0;
          ...
          plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], true);
  ...
  else if(code_seen('M'))
  {
    ...
    case 601: //M601 Pause in UltiLCD2, X[pos] Y[pos] Z[relative lift] L[later retract distance]
    {
        ...
        //Only move to the last position if we are still printing. Else we aborted.
...
void manage_inactivity()
{
  ...
  if (READ(SAFETY_TRIGGERED_PIN))
    ...
    if(degHotend(active_extruder)>EXTRUDER_RUNOUT_MINTEMP)
    {
     ...
     previous_millis_cmd=millis();
     plan_set_e_position(oldepos, true);
